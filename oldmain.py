...
class CreateRoomWindow:
    ...
    def create_room(self):
        ...
        room_port = self.get_room_port()
        password = self.get_password()
    
        if not self.validate_inputs(room_name, room_port, password):
            return
    
        self.start_server(room_port)
        self.start_ngrok(room_port)
        self.open_chat_window(room_name)
    
    def get_room_name(self):
        return self.room_name_entry.get()
    
    def get_room_port(self):
        return self.room_port_entry.get()
    
    def get_password(self):
        return self.password_entry.get() if self.password_checkbox_var.get() else None
    
    def validate_inputs(self, room_name, room_port, password):
        if len(room_name) < 2:
            messagebox.showerror("Invalid Room Name", "Room name must be at least 2 characters.")
            return False
        if not room_port.isdigit() or int(room_port) < 1 or int(room_port) > 65535:
            messagebox.showerror("Invalid Port", "Port must be between 1 and 65535.")
            return False
        if password and len(password) > 12:
            messagebox.showerror("Invalid Password", "Password must be 12 characters or less.")
            return False
        return True
    
    def start_server(self, room_port):
